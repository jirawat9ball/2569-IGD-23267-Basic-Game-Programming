...
public class Assignment_SubmitterEditor : Editor
{
    ...
    private static bool ValidateSubmitter(Assignment_Submitter submitter)
    {
        ...
        if (submitter.section == Assignment_Submitter.StudentSection.Other && string.IsNullOrEmpty(submitter.customSection))
        {
    ...
    private static void RunTestsAndSubmit(Assignment_Submitter submitter)
    {
        ...
        receiver.sectionString = (submitter.section == Assignment_Submitter.StudentSection.Other)
            ? submitter.customSection
            : submitter.section.ToString().Replace("_", " ");
        receiver.weekName = submitter.weekName;

        var prop = typeof(Assignment_Submitter).GetProperty("googleSheetWebAppURL",
