...
[CustomEditor(typeof(Assignment_Submitter_Week04))]
public class Assignment_SubmitterEditor_Week04 : Editor
{
    [MenuItem("Assignment/Submit Score Week 04 %#h")]
    public static void SubmitScoreFromMenu()
    {
        var submitter = Object.FindAnyObjectByType<Assignment_Submitter_Week04>();
    ...
    public override void OnInspectorGUI()
    {
        ...
        Assignment_Submitter_Week04 submitter = (Assignment_Submitter_Week04)target;
        ...
        if (GUILayout.Button("📤 ส่งงานเข้า Google Sheet (Week 04)", buttonStyle))
        {
            if (!ValidateSubmitter(submitter))
                return;
    ...
    private static bool ValidateSubmitter(Assignment_Submitter_Week04 submitter)
    {
        if (submitter == null)
        {
            Debug.LogError("❌ ไม่พบ GameObject ที่มี Assignment_Submitter_Week04 ใน Scene! กรุณาเพิ่มลงใน Scene ก่อน");
            return false;
        }
        ...
        if (submitter.section == Assignment_Submitter_Week04.StudentSection.Other && string.IsNullOrEmpty(submitter.customSection))
        {
    ...
    private static void RunTestsAndSubmit(Assignment_Submitter_Week04 submitter)
    {
        Debug.Log("⏳ กำลังคำนวณคะแนนจากการรันเทส Week 04...");

        var receiver = ScriptableObject.CreateInstance<TestRunnerCallback_Week04>();
        ...
        receiver.sectionString = (submitter.section == Assignment_Submitter_Week04.StudentSection.Other)
            ? submitter.customSection
            : submitter.section.ToString().Replace("_", " ");
        receiver.weekName = submitter.weekName;

        var prop = typeof(Assignment_Submitter_Week04).GetProperty("googleSheetWebAppURL",
...
[System.Serializable]
public class TestRunnerCallback_Week04 : ScriptableObject, ICallbacks
{
    ...
    private void SendScoreToGoogleSheet(string score, string maxScore)
    {
        ...
        updateCallback = () =>
        {
            if (www.isDone)
            {
                ...
                {
                    UnityEngine.Debug.Log("<color=green>✅ ส่งงานเรียบร้อยแล้ว!</color> คะแนน " + score + "/" + maxScore + " ถูกบันทึกลง Google Sheet แท็บ Week04");
                }
                www.Dispose();
